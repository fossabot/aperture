...
type responseMetricsRegistry struct {
	...
	workloadHistogramVec  *prometheus.HistogramVec
...
func ProvideResponseMetricsAPI(registry *prometheus.Registry, lifecycle fx.Lifecycle) iface.ResponseMetricsAPI {
	...
	rm.workloadHistogramVec = prometheus.NewHistogramVec(prometheus.HistogramOpts{
	...
	lifecycle.Append(fx.Hook{
		OnStart: func(_ context.Context) error {
			var errs error
			err := registry.Register(rm.workloadHistogramVec)
			if err != nil {
				if are, ok := err.(prometheus.AlreadyRegisteredError); ok {
					...
					rm.workloadHistogramVec = are.ExistingCollector.(*prometheus.HistogramVec)
				} else {
		...
		OnStop: func(_ context.Context) error {
			unregistered := registry.Unregister(rm.workloadHistogramVec)
			if !unregistered {
...
func (rm *responseMetricsRegistry) GetTokenLatencyHistogram(labels map[string]string) (prometheus.Observer, error) {
	tokenHistogram, err := rm.workloadHistogramVec.GetMetricWith(labels)
	if err != nil {
...
func (rm *responseMetricsRegistry) DeleteTokenLatencyHistogram(policyName, policyHash, componentIndex string) bool {
	labels := make(map[string]string)
	labels[metrics.PolicyNameLabel] = policyName
	labels[metrics.PolicyHashLabel] = policyHash
	labels[metrics.ComponentIndexLabel] = componentIndex

	deleted := rm.workloadHistogramVec.DeletePartialMatch(labels)
